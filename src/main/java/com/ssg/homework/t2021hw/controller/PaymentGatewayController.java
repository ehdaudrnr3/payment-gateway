...
public class PaymentGatewayController {
	...
	public PaymentDto approve(
			@RequestParam(required = true) String mbrId,
			@RequestParam(required = true) String pmtCode,
			@RequestParam(required = false) String pmtType,
			@RequestParam(required = true) long pmtAmt) {
		PaymentRequest request = PaymentRequest.ofApproval(mbrId, pmtCode, pmtType, pmtAmt);
	...
	public PaymentDto cancel(
			@RequestParam(required = true) String mbrId,
			@RequestParam(required = true) String bfPmtId,
			@RequestParam(required = true) long pmtAmt) {
		PaymentRequest request = PaymentRequest.ofCancel(mbrId, bfPmtId, pmtAmt);
	...
	@ResponseBody
	public PaymentDto partialCancel(
			@RequestParam(required = true) String mbrId,
			@RequestParam(required = true) String bfPmtId,
			@RequestParam(required = true) long pmtAmt) {
		PaymentRequest request = PaymentRequest.ofPartialCancel(mbrId, bfPmtId, pmtAmt);
		return paymentService.cancel(request);
	}
	...
	@ResponseBody
	public List<PaymentDto> getRecentPaymentList(
			@RequestParam(required = true) String mbrId,
			@RequestParam(required = false) String succYn,
			@RequestParam(defaultValue = "15") Integer size) {
		return paymentService.getRecentPayments(mbrId, succYn, size);
	}
